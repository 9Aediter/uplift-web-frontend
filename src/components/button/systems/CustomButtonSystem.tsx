// No "use client" - SSR-compatible
import { ButtonSystem, BaseButtonConfig, createButtonComponent, BaseButtonProps } from '../core/BaseButton'

// Custom Button System Configuration (Gradient buttons)
const customButtonConfig: BaseButtonConfig = {
  baseStyles: "relative font-medium rounded-md inline-flex items-center justify-center transition-all duration-300 overflow-hidden group whitespace-nowrap gap-2",
  variants: {
    primary: "bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 hover:from-cyan-600 hover:to-blue-600",
    secondary: "bg-gradient-to-r from-fuchsia-500 to-pink-500 text-white shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 hover:from-fuchsia-600 hover:to-pink-600",
    outline: "border-2 border-cyan-500 text-cyan-600 bg-transparent hover:bg-cyan-50 hover:text-cyan-700 dark:hover:bg-cyan-950/20 dark:text-cyan-400 dark:border-cyan-400 dark:hover:text-cyan-300"
  },
  sizes: {
    sm: "h-8 px-3 py-1 text-sm",
    md: "h-10 px-4 py-2 text-base",
    lg: "h-12 px-6 py-3 text-lg"
  },
  defaultVariant: "primary",
  defaultSize: "md"
}

// Custom Button System Class
export class CustomButtonSystem extends ButtonSystem {
  constructor() {
    super(customButtonConfig)
  }

  createComponent() {
    return createButtonComponent(this)
  }

  // Additional Custom-specific methods
  getGradientVariants(): string[] {
    return ['primary', 'secondary']
  }

  getOutlineVariants(): string[] {
    return ['outline']
  }

  // Method to get gradient color info
  getGradientInfo(variant: string): { from: string; to: string } | null {
    switch (variant) {
      case 'primary':
        return { from: 'cyan-500', to: 'blue-500' }
      case 'secondary':
        return { from: 'fuchsia-500', to: 'pink-500' }
      default:
        return null
    }
  }
}

// Export the system instance and component
export const customButtonSystem = new CustomButtonSystem()
export const CustomButton = customButtonSystem.createComponent()

// Type exports
export type CustomButtonVariant = 'primary' | 'secondary' | 'outline'
export type CustomButtonSize = 'sm' | 'md' | 'lg'

export interface CustomButtonProps extends BaseButtonProps {
  variant?: CustomButtonVariant
  size?: CustomButtonSize
}