...
import useLandingStore from '@/lib/store/landing-store';
import { fetchAllServices } from '@/lib/api/service-api';
import type { Service } from '@/lib/store/landing-store'; // Ensure Service type is imported

const TestServicePage = () => {
  // Part 1: Data from the Zustand Landing Store
  const { Service: storeServices, isLoading: storeIsLoading, error: storeError, fetchLandingData } = useLandingStore();
  ...
  useEffect(() => {
    const fetchRawData = async () => {
      ...
      setRawApiError(null);
      try {
        const data = await fetchAllServices(); // This now returns raw Strapi data
        setRawApiData(data);
      } catch (error) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      {/* Column 1: Data from Zustand Landing Store */}
      <div className="p-6 bg-gray-800/50 rounded-lg border border-gray-700">
        <h2 className="text-2xl font-semibold mb-4 text-cyan-400">1. Data from Landing Store (Services)</h2>
        {storeIsLoading && <p>Loading from store...</p>}
        {storeError && <p className="text-red-500">Error: {storeError}</p>}
        {storeServices && !storeIsLoading && (
          <div>
            <p className="mb-2">Services loaded in store: {storeServices[0]?.Service?.length || 0}</p>
            {renderJson(storeServices[0]?.Service)}
          </div>
        )}
      </div>
      ...
      <div className="p-6 bg-gray-800/50 rounded-lg border border-gray-700">
        <h2 className="text-2xl font-semibold mb-4 text-lime-400">2. Raw Data from Service API</h2>
